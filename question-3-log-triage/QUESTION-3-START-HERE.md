...
> **Important:** Only count entries with level **ERROR**. Ignore WARN and INFO entries even if they contain error codes.
...
Your solution goes in **`triage.ps1`** in this folder. Open that file and write your code there. The variable `$LogsDir` is already set to point at the `logs/` folder.
...
**Output verification is the test for this problem.** Run against the sample logs and verify the output makes sense. Unit tests are optional.
